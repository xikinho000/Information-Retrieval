...
public class Compression {
    ...
    public static void blockingCompression(
            TreeMap<String, DictionaryClass> info, boolean frontCoding
    ){
        int k = 8, wordCount = 0; 
        long totalCount = 0;
        ...
        while(j < longString.length() - 1) {
            if (Character.isDigit(longString.charAt(j)) && 
                Character.isDigit(longString.charAt(j + 1))) {
                    longDictionaryTerms.add(
                        longString.charAt(j) + "" + longString.charAt(j + 1)
                    );
                j++;
            }
            else 
        ...
        if (frontCoding) {
            ...
            while (ptr <  termPtr.size()) {
                if (ptr ==  termPtr.size() - 1){
                    frontCoding(longDictionaryTerms.subList(
                        termPtr.get(ptr).intValue(), 
                        longDictionaryTerms.size() - 1)
                    );
                }
                else{
                    frontCoding(longDictionaryTerms.subList(
                    termPtr.get(ptr).intValue(), 
                    termPtr.get(ptr + 1).intValue() - 1));
                }
                ptr++;
            }
        }
    ...
    public static void frontCoding (List<String> longDictionaryTerms) 
    {
        ...
        while (jPtr < longDictionaryTerms.size()) {
            ...
            if (jPtr == 1)
                    jPtr = iPtr + 1;
            while (jPtr < longDictionaryTerms.size() && 
                longDictionaryTerms.get(tmpiPtr)
                .equals(longDictionaryTerms.get(jPtr))
            ){
                tmpiPtr++;
                jPtr++;
            }
            ...
            if (help < tmpiPtr) {
                storeResult.addAll(
                    longDictionaryTerms.subList(help, tmpiPtr));
                help = tmpiPtr;
                storeResult.add("*");
                help++;
                storeResult.addAll(
                    longDictionaryTerms.subList(tmpiPtr, iPtr));
            ...
            remain = "$";
            while(jPtr < longDictionaryTerms.size() && 
                !isDigit(longDictionaryTerms.get(jPtr)))
            {
